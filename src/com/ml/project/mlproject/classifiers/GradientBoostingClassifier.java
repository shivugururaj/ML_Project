...
public class GradientBoostingClassifier {
	...
	public double classify() throws IOException {
		...
		double accuracy = predicationLabels.filter(pl -> pl._1().equals(pl._2())).count() / ((double) validation.count() -1000) ;
		...
		for (int i = 0; i < metrics.labels().length; i++) {
			...
			buffer.append("Class ").append(metrics.labels()[i]).append(" = ")
					.append(metrics.precision(metrics.labels()[i])).append("\n");
			buffer.append("Class ").append(metrics.labels()[i]).append(" = ")
					.append(metrics.recall(metrics.labels()[i])).append("\n");
			buffer.append("Class ").append(metrics.labels()[i]).append(" = ")
					.append(metrics.fMeasure(metrics.labels()[i])).append("\n");
